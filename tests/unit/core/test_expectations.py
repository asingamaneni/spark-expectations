...
class TestValidateRules:
    ...
    def test_validate_rules_raises_error_when_null_in_any_row(self, spark, rules_df_with_partial_null_rows):
    ...
    def test_validate_rules_error_message_lists_columns_sorted(self, spark, rules_df_schema, rule_data_all_null_ids):
...
class TestAddHashColumns:
    ...
    def test_add_hash_columns_handles_null_rule(self, spark, se_instance, rules_df_schema, rule_data_null_rule):
...
class TestAnsiModeWarning:
    ...
    def test_warn_if_ansi_mode_enabled_logs_warning_for_spark4(self, spark, rules_df_schema, base_rule_data, caplog):
        """When SPARK_MINOR_VERSION >= 4.0 and ANSI mode is on, a warning is logged."""
        rules_df = spark.createDataFrame(base_rule_data, schema=rules_df_schema)
        writer = WrappedDataFrameWriter().mode("append").format("parquet")

        with patch("spark_expectations.core.expectations.SPARK_MINOR_VERSION", 4.0):
    ...
    def test_warn_if_ansi_mode_not_logged_for_spark3(self, spark, rules_df_schema, base_rule_data, caplog):
        """When SPARK_MINOR_VERSION < 4.0, no ANSI mode warning is logged."""
        rules_df = spark.createDataFrame(base_rule_data, schema=rules_df_schema)
        writer = WrappedDataFrameWriter().mode("append").format("parquet")

        with patch("spark_expectations.core.expectations.SPARK_MINOR_VERSION", 3.5):
            with caplog.at_level(logging.WARNING, logger="spark_expectations"):
                se = SparkExpectations(
                    product_id="test_product",
                    rules_df=rules_df,
