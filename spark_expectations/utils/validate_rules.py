...
class SparkExpectationsValidateRules:
    ...
    @staticmethod
    def _validate_single_subquery(sq: sqlglot_expr.Subquery) -> None:
        """
        ...
        Args:
        sq (sqlglot_expr.Subquery): The subquery node to validate.

        Raises:
        SparkExpectationsInvalidRowDQExpectationException: If any of the checks fail.
        """
        inner = sq.this if sq.this else sq
        if not isinstance(inner, sqlglot_expr.Select):
            raise SparkExpectationsInvalidRowDQExpectationException(
                "[row_dq] Subquery does not contain SELECT statement"
            )

        # sqlglot >=23 changed the AST API: args.get("from") no longer works.
        # Use find(exp.From) which is stable across both old and new versions.
        from_node = inner.find(sqlglot_expr.From)
        if from_node is None:
            raise SparkExpectationsInvalidRowDQExpectationException("[row_dq] Subquery does not contain FROM")

        source = from_node.this
        if not isinstance(source, (sqlglot_expr.Table, sqlglot_expr.Subquery, sqlglot_expr.Join)):
            raise SparkExpectationsInvalidRowDQExpectationException(
    ...
    @staticmethod
    def validate_subqueries(tree: sqlglot_expr.Expression) -> None:
        """
        ...
        Args:
        tree (sqlglot_expr.Expression): Parsed SQL expression to inspect.
    ...
    @staticmethod
    def get_subqueries(tree: sqlglot_expr.Expression) -> list:
        """
        ...
        Args:
            tree (sqlglot_expr.Expression): Parsed SQL expression tree.

        Returns:
            list: List of unique subquery/query expressions.
        """
        subqueries = []
        for expression in tree.find_all(sqlglot_expr.Subquery, sqlglot_expr.Query):
            # Skip Query nodes that are direct children of a Subquery (already captured)
            if isinstance(expression, sqlglot_expr.Query) and isinstance(expression.parent, sqlglot_expr.Subquery):
                continue
            subqueries.append(expression)
        return subqueries

    @staticmethod
    def check_query_dq(tree: sqlglot_expr.Expression) -> bool:
        """
        ...
        returns True if the underlying node is a sqlglot_expr.Select; otherwise False.

        Args:
        tree (sqlglot_expr.Expression): Parsed SQL expression (e.g., from sqlglot.parse_one).

        Returns:
        bool: True if the expression resolves to a SELECT, False otherwise.
        """
        if isinstance(tree, sqlglot_expr.Select):
            return True
        inner = tree.this
        if isinstance(inner, (sqlglot_expr.Subquery, sqlglot_expr.Select)):
            return SparkExpectationsValidateRules.check_query_dq(inner)
        return False

    @staticmethod
    def check_agg_outside_subqueries(tree: sqlglot_expr.Expression, agg_funcs: list) -> bool:
        """
        ...
        for query in subqueries:
            query_agg = list({node.key for node in query.find_all(sqlglot_expr.AggFunc)})
    ...
    def validate_row_dq_expectation(df: DataFrame, rule: Dict, raise_exception: bool = False) -> ValidationResult:
        """
        ...
        try:
            ...
            agg_funcs = list({node.key for node in tree.find_all(sqlglot_expr.AggFunc)})
            ...
            return ValidationResult(is_valid=False, error_message=error_msg, rule=rule, rule_type="row_dq")
        ...
        if check_subqueries:
            ...
            try:
                ...
                if raise_exception:
                    raise
                return ValidationResult(is_valid=False, error_message=str(e), rule=rule, rule_type="row_dq")
    ...
    @staticmethod
    def validate_agg_dq_expectation(df: DataFrame, rule: Dict, raise_exception: bool = False) -> ValidationResult:
        """
        ...
        try:
            ...
            return ValidationResult(is_valid=False, error_message=error_msg, rule=rule, rule_type="agg_dq")
        if not agg_funcs:
        ...
        if is_composite_query:
            ...
            for key, subquery in dynamic_query_kv_pairs.items():
                if not re.search(r"\bselect\b.*\bfrom\b", subquery, re.IGNORECASE):
                    ...
                    return ValidationResult(is_valid=False, error_message=error_msg, rule=rule, rule_type="query_dq")
    ...
    def validate_expectations(
        ...
        for rule in rules:
            ...
            try:
                ...
                logger.warning(
                    "Invalid rule detected: rule='%s', error='%s'", rule.get("rule", "unknown"), result.error_message
                )
        ...
        if total_invalid > 0:
            ...
            for rule_type_str, results in invalid_results.items():
                logger.warning("  - %s: %d invalid rule(s)", rule_type_str, len(results))
        else:
            logger.info("Rule validation complete: all %d rules are valid", len(rules))
